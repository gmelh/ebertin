...
{
    public class TriDialControl : Control
    {
        ...
        private void DrawStaticElements90(DrawingContext context, Point center, double radius)
        {
            ...
            double planetsCircleRadius = radius * 0.73;
            
            // Calculate innermost circle radius based on graduation mark length
            double markLength = (outerRadius - innerRadius) * 0.2;
            double innerInnerRadius = planetsCircleRadius - markLength;
            
            // Calculate the third pair of circles with space for planets
            double planetSpaceGap = innerRadius - planetsCircleRadius;
            double thirdCircleRadius = innerInnerRadius - planetSpaceGap;
            double fourthCircleRadius = thirdCircleRadius - markLength;
            ...
            context.DrawEllipse(null, pen, center, planetsCircleRadius, planetsCircleRadius);
            context.DrawEllipse(null, pen, center, innerInnerRadius, innerInnerRadius);
            context.DrawEllipse(null, pen, center, thirdCircleRadius, thirdCircleRadius);
            context.DrawEllipse(null, pen, center, fourthCircleRadius, fourthCircleRadius);
            ...
            // Draw inner tick marks (between planets circle and innermost circle)
            for (int degrees = 0; degrees < 360; degrees += 4)
            {
                double radians = DegreesToRadians(90 + degrees);
                bool isLongMark = degrees == 0 || degrees % 20 == 0;
                double innerMarkLength = isLongMark ? 
                    (planetsCircleRadius - innerInnerRadius) : // Full distance for long marks
                    (planetsCircleRadius - innerInnerRadius) / 2; // Half distance for short marks

                var startPoint = new Point(
                    center.X + innerInnerRadius * Math.Cos(radians),
                    center.Y + innerInnerRadius * Math.Sin(radians)
                );

                var endPoint = new Point(
                    center.X + (innerInnerRadius + innerMarkLength) * Math.Cos(radians),
                    center.Y + (innerInnerRadius + innerMarkLength) * Math.Sin(radians)
                );

                var markPen = new Pen(Brushes.White, isLongMark ? 2.5 : 1);
                context.DrawLine(markPen, startPoint, endPoint);
            }
            
            // Draw third set of inner tick marks (between third and fourth circles)
            for (int degrees = 0; degrees < 360; degrees += 4)
            {
                double radians = DegreesToRadians(90 + degrees);
                bool isLongMark = degrees == 0 || degrees % 20 == 0;
                double innerMarkLength = isLongMark ? 
                    (thirdCircleRadius - fourthCircleRadius) : // Full distance for long marks
                    (thirdCircleRadius - fourthCircleRadius) / 2; // Half distance for short marks

                var startPoint = new Point(
                    center.X + fourthCircleRadius * Math.Cos(radians),
                    center.Y + fourthCircleRadius * Math.Sin(radians)
                );

                var endPoint = new Point(
                    center.X + (fourthCircleRadius + innerMarkLength) * Math.Cos(radians),
                    center.Y + (fourthCircleRadius + innerMarkLength) * Math.Sin(radians)
                );
            ...
            // Plot planets
            PlotPlanetsInside(context, center, planetsCircleRadius, innerRadius);
        }
