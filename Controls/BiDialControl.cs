...
{
    public class BiDialControl : Control
    {
        ...
        private void DrawStaticElements90(DrawingContext context, Point center, double radius)
        {
            ...
            double planetsCircleRadius = radius * 0.73; // Moved slightly further from inner circle (was 0.75)
            
            // Calculate innermost circle radius based on graduation mark length
            double markLength = (outerRadius - innerRadius) * 0.2;
            double innerInnerRadius = planetsCircleRadius - markLength; // Just a tick mark's distance away
            ...
            // All tick marks are the same length, but longer ones are still thicker
            for (int degrees = 0; degrees < 360; degrees += 4)
            {
                double radians = DegreesToRadians(90 + degrees);
                bool isLongMark = degrees == 0 || degrees % 20 == 0;
            ...
            // Plot planets - now between planetsCircle and innerCircle
        ...
        private void DrawStaticElements120(DrawingContext context, Point center, double radius)
        {
            double outerRadius = radius;
            double innerRadius = radius * 0.885;
            double planetsCircleRadius = radius * 0.73; // Same as in 90-degree dial
